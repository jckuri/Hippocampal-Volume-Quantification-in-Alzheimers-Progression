...
def LoadHippocampusData(root_dir, y_shape, z_shape):
    ...
    for f in images:
        ...
        n_slices.append(image.shape[0])
    max_slices = max(n_slices)
    print(f'max_slices = {max_slices}, len(images) = {len(images)}')
...
#         label = med_reshape(label, new_shape=(label.shape[0], y_shape, z_shape)).astype(int)
        ...
        image = med_reshape(image, new_shape=(max_slices, y_shape, z_shape))
        label = med_reshape(label, new_shape=(max_slices, y_shape, z_shape)).astype(int)
        
        #print(f'image.shape={image.shape}, label.shape={label.shape}')

        # TASK: Why do we need to cast label to int?
        # ANSWER: Because labels are 0, 1, or 2. Nothing else. Not real numbers. 
        # Only integers representing 3 categories: No hippocampus, hippocampus 1, hippocampus 2.
